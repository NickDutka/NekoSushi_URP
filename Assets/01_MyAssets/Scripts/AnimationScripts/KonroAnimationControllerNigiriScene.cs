...
public class KonroAnimationControllerNigiriScene : MonoBehaviour
{
    ...
    [SerializeField] StepManagerNigiriScene stepManagerNigiriScene;
    ...
    private void Update()
    {
        ...
        //
        if (stepManagerNigiriScene.currentStep < 2)
        {
            SetTalkingState(true);
        }

        // Telling you to grab Yanagiba
        if (stepManagerNigiriScene.currentStep == 2)
        {
            SetAngryState(false);
            SetTalkingState(false);
        ...
        if (stepManagerNigiriScene.currentStep == 3)
        {
            SetPointTalkingState(false);
            SetAngryState(true);
        }
        // If you grab the right knife, konro is happy
        if (stepManagerNigiriScene.currentStep >= 4)
        {
            SetAngryState(false);
            SetPointTalkingState(false);
            SetTalkingState(true);
        }
        // Telling you to cut Salmon
        if (stepManagerNigiriScene.currentStep == 5)
        {
            SetTalkingState(false);
            SetPointTalkingState(true);
        }
